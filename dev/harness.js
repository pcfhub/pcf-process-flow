...
(function () {
     ...
     */
    /**
     * What `attributes.Options` carries, per the "Column options" switch.
     *
     * `mixed` is the default because it is the ordinary case: a colour is
     * optional in the choice editor, so most real columns have some options
     * coloured and some not. A fixture where every option is coloured never
     * shows the fallback, and one where none is never shows the colours.
     */
    var COLUMN_OPTIONS = {
        mixed: [
        ...
        coloured: [
            { Value: 1, Label: 'Qualify', Color: '#e8502a' },
            { Value: 2, Label: 'Develop', Color: '#0f6cbd' },
            { Value: 3, Label: 'Propose', Color: '#107c10' },
            { Value: 4, Label: 'Close', Color: '#5b2d90' },
        ],
        plain: [
    ...
    /**
     * Put the platform's copy of the column into the shape the bound column
     * would actually hand over.
     *
     * `columnValue` is kept as an array here because that is the one shape that
     * can express every state; the platform is not so tidy. A Multi-Select
     * Choice column sends an array, and a Choice column sends either a bare
     * number or — through this control's type group, on a real form — an object
     * with the value buried in `_val`. All three have to be reachable, because
     * a control that reads only the documented one renders an unmarked bar on
     * the host it is most likely to be deployed to.
     */
    function shapeValue(value) {
        ...
        if (document.getElementById('harness-columntype').value === 'multiselectpicklist') {
            return value;
        }

        var first = value.length > 0 ? value[value.length - 1] : null;

        if (first === null) {
            return null;
        }

        return document.getElementById('harness-valueshape').value === 'wrapped'
            ? { _val: first, _label: String(first), _state: -1 }
            : first;
    }

    /**
     * The comma-separated field, as the array `columnValue` is kept in.
     *
     * Empty entries are dropped BEFORE the conversion, because `Number('')` is
     * `0` and `0` is a perfectly finite number — so an empty box produced a
     * selection of `[0]` rather than an empty one, and the bar reported a
     * stage nothing had selected.
     */
    function parseValue(text) {
        return String(text)
            ...
            .map(Number)
            .filter(function (entry) { return Number.isFinite(entry); });
    }
    ...
    function options() {
        return {
            ...
            calls: calls,
            value: shapeValue(columnValue),
            columnType: document.getElementById('harness-columntype').value,
            columnOptions: COLUMN_OPTIONS[document.getElementById('harness-columnoptions').value],
            options: document.getElementById('harness-options').value || null,
            exclude: document.getElementById('harness-exclude').value || null,
            defaultColor: document.getElementById('harness-defaultcolor').value || null,
    ...
    function notifyOutputChanged() {
        ...
        window.setTimeout(function () {
            ...
            if (Object.prototype.hasOwnProperty.call(outputs, 'value') && outputs.value !== undefined) {
                // Normalised back to the array this page keeps. The control is
                // free to have written a number, and did if the column is a
                // Choice column — which is the round trip worth watching.
                columnValue = outputs.value === null
                    ? null
                    : parseValue(Array.isArray(outputs.value) ? outputs.value.join(',') : outputs.value);

                document.getElementById('harness-value').value =
                    columnValue === null ? '' : columnValue.join(',');
            }
    ...
    window.__harnessStart = function () {
        ...
        /*
         * And `input` as well, for the text fields the panel has grown — the
         * exclude list and the options override are things you want to watch
         * take effect as you type, and `change` on a text input does not fire
         * until it loses focus. `harness-value` is skipped because it has its
         * own handler below: it feeds the platform's copy of the column rather
         * than an input property, which is a different event entirely.
         */
        document.querySelector('.harness-controls').addEventListener('input', function (event) {
            if (event.target.id !== 'harness-value') {
                render();
            }
        });
        ...
        document.getElementById('harness-value').addEventListener('input', function (event) {
            columnValue = parseValue(event.target.value);
            render();
        });
