...
/**
 * `parentElement`, which is `parentNode` for everything this file builds —
 * there are no text or comment nodes here, so the two can never disagree.
 * Present because a control written against the real DOM reaches for the more
 * specific one, and an `undefined` would silently make a class toggle a no-op.
 */
