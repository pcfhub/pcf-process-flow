...
(function (root, factory) {
     ...
     *
     * The keys are the ones in `strings/ProcessFlow.1033.resx`, and a key that
     ...
     */
    var STRINGS = {
        ProcessFlow_Name: 'Process Flow',
        ProcessFlow_NoAccess: 'You do not have access to this value.',
        ProcessFlow_NoSteps: 'No steps to show.',
        ProcessFlow_NoStage: 'No stage reached',
        ProcessFlow_StateDone: 'completed',
        ProcessFlow_StateCurrent: 'current stage',
        ProcessFlow_StateFuture: 'not reached',
    };

    /**
     * The option list a model-driven form publishes as
     * `value.attributes.Options`, in the display order the maker set in the
     * choice editor — which is where the step order comes from.
     *
     * Two of the four carry no `Color`, because that is the ordinary case: a
     * colour is optional in the choice editor and most options have none. A
     * fixture where every option is coloured never exercises the fallback the
     * control exists to provide.
     */
    var OPTIONS = [
        { Value: 1, Label: 'Qualify', Color: '#e8502a' },
        { Value: 2, Label: 'Develop', Color: '#0f6cbd' },
    ...
    var DEFAULTS = {
         ...
         *
         * Three shapes reach a control bound through this type group, and all
         * three have to be reachable here:
         *
         *   - an array of numbers, from a Multi-Select Choice column
         *   - a bare number, the documented single-select shape
         *   - `{ _val: 1, _label: 'Qualify', _state: -1 }`, which is what a
         *     single-select column ACTUALLY hands over through a type group on
         *     a real form. A control reading only `typeof raw === 'number'`
         *     renders an unmarked bar, and nothing else here would catch it.
         */
        value: [1, 2, 3],

        /**
         * `attributes.Type`, which is the only reliable discriminator for the
         * column's arity — `property.type` reports "MultiSelectOptionSet" for
         * every binding through a type group, whichever column is bound.
         *
         * `''` reproduces a host that publishes the attribute bag without a
         * usable Type, where the control has only the value's own shape to go
         * on. Reachable independently of `publishesMetadata`, which removes the
         * bag entirely.
         */
        columnType: 'multiselectpicklist',

        /** What `value.attributes.Options` carries. `[]` is a column with none. */
        columnOptions: OPTIONS,

        /*
         * The control's three input properties.
         *
         * Given switches of their own rather than left to `inputs`, because the
         * control reads all three on every render — an absent one is a
         * TypeError, not a default — and because the exclude list in particular
         * is the thing most worth flipping while looking at the bar.
         */

        /** The `options` override. `null` falls through to column metadata. */
        options: null,
        /** The `exclude` list: option values or labels, comma separated. */
        exclude: null,
        /** The `defaultColor` input, for an option whose metadata carries none. */
        defaultColor: '#e8502a',
        ...
        return {
            ...
            parameters: Object.assign({
                value: {
                     ...
                     *
                     * The control reads `attributes?.Options` for the steps and
                     * `attributes?.Type` for the arity, and that single `?` is
                     * the whole canvas/model-driven difference. Supplying it on
                     * canvas would hide the one bug this switch exists to find:
                     * a control that renders an empty bar wherever the platform
                     * publishes no metadata.
                     *
                     * `Type` is deliberately NOT on the declared `Metadata`
                     * interface. The runtime object carries considerably more
                     * than the type definitions admit, and this is one of them —
                     * so the control casts to read it, and a host that omits it
                     * has to be reachable here to prove the cast is guarded.
                     */
                    attributes: host.publishesMetadata
                        ? {
                            Type: o.columnType,
                            Options: o.columnOptions,
                            LogicalName: 'stage',
                            DisplayName: o.label,
                        }
                        : undefined,
                    ...
                    /*
                     * The type-group's accepted member, and a liar.
                     *
                     * A real form reports "MultiSelectOptionSet" here for EVERY
                     * binding through this group — observed on a single-select
                     * `address1_addresstypecode` column reporting
                     * `attributes.Type: "picklist"` at the same moment. It is
                     * reproduced faithfully so that a control which reaches for
                     * it instead of `attributes.Type` fails here rather than on
                     * somebody's form.
                     */
                    type: 'MultiSelectOptionSet',
                },
                options: { raw: o.options, type: 'Multiple' },
                exclude: { raw: o.exclude, type: 'SingleLine.Text' },
                defaultColor: { raw: o.defaultColor, type: 'SingleLine.Text' },
            }, parameters),
     ...
     *
     * `pcf-scripts` emits `registerControl('PCFHub.ProcessFlow', ctor)`
